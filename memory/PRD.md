...
- **Session 21 (Mar 22, 2026)**: CRITICAL fixes — (1) Fixed Barrister View filter bug: was rejecting ALL reports because it filtered for `status === "completed"` but DB stores `null` — now accepts any report with analysis content. (2) Added Table of Contents to Barrister View with section headings and "Synthesised from N reports" badge. (3) Rewrote aggressive mode prompt from "double word count" to genuine assertive advocacy tone — confident declarations, no hedging, draft submission paragraphs. (4) Fixed sentence extraction in both ReportView and BarristerView — strips analysis text after the actual sentence. (5) Fixed "25%" to "~40%" on Appeal Statistics page "Hidden Tragedy" section. (6) Fixed hero section readability — 95% overlay, doubled badge opacity. Testing: iteration_78 — 100% frontend.
- **Session 20 (Mar 21, 2026)**: Added interactive state-by-state appeal stats widget (8 states, filed/allowed/dismissed/pending, ranked grounds, crime types, conviction vs sentence split). Fixed iOS Safari PDF/Print using navigator.share() API with fallback. Testing: iteration_77 — 100% frontend.
