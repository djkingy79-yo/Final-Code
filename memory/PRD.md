...
- **AI Auto-Detection of Case Metadata (29 Mar 2026):** Critical fix — new cases were defaulting to "Homicide" regardless of uploaded document content. Added AI auto-detection that runs: (1) after "Extract All Text to Case", and (2) at the start of report generation as fallback. The LLM reads document text and identifies offence_category, offence_type, sentence, state, court, and case_number. Case metadata is automatically updated in the database. Frontend shows toast with detected info and refreshes the case view. Tested: R v Karlsson (sexual_offences case) correctly auto-detected from document content.
