...
- **Grounds of Merit Paywall (29 Mar 2026):** Enforced strict paywall — before $99 payment, ONLY the count is shown ("X Grounds Found! Unlock for $99"). No titles, descriptions, badges, investigate buttons, or search buttons are visible. The backend returns an empty grounds array with just the count when locked. Admin bypass preserved (admin gets everything free). After payment, full ground cards with titles, investigate button, deep analysis are available.
- **Auto-Generate Timeline + Summary (29 Mar 2026):** After document upload, a background task auto-generates: (1) case summary (3-5 sentences), and (2) chronological timeline events from document content. Both are stored in DB and immediately visible in the UI.
- **Investigate Ground Export (29 Mar 2026):** Added Print and PDF View buttons to the ground detail dialog. After clicking Investigate (which triggers deep AI analysis), users can print or save the full ground analysis as PDF — includes title, badges, description, supporting evidence, law sections, similar cases, and deep investigation analysis with NOT LEGAL ADVICE disclaimer.
