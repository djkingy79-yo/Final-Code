...
.emergent/                  Legacy Emergent platform config (retained, not used in prod)
...
| `.emergent/emergent.yml`, `.emergent/summary.txt` | Legacy Emergent platform config | Platform was replaced with self-hosted; files are inert but IDENTITY_LOCK and CI prohibit Emergent hostnames |
...
- **`docs/audit/` directory** — Contains historical git-log artifacts generated by the previous development platform (Emergent). Not application code or user docs. Can be deleted or archived.
- **`.emergent/` directory** — Legacy platform config. IDENTITY_LOCK and CI already block Emergent usage. The directory is inert; safe to delete.
...
| # | Action | Rationale |
|---|--------|-----------|
| L1 | **Delete `.emergent/` directory** | Inert legacy config. IDENTITY_LOCK and CI scans already prevent Emergent re-introduction. Clean it up to reduce noise. |
| L2 | **Delete `docs/audit/` directory** | Historical debug artifacts from the Emergent development platform. Not user-facing documentation. |
| L3 | **Remove frontend mockup files** (`public/mockups.html`, `public/mockups-mobile.html`) | Development artifacts served from `public/` unnecessarily increase the production build size and could confuse visitors. |
| L4 | **Audit and tree-shake unused Shadcn/UI primitives** in `frontend/src/components/ui/` | Reduces frontend bundle size. Low risk — Shadcn components are self-contained. |
| L5 | **Keep `md_normaliser.py` and `mdRender.js` in sync** by adding a test that runs both on the same input and asserts identical output | These files are intentionally mirrored but have no automated sync check. A divergence would cause backend-saved text to render differently on the frontend. |
| L6 | **Add a `MIN_HEALTHY_ISSUE_COUNT` constant to `pipeline_orchestrator.py`** and document the rationale | Low-effort, high-clarity improvement. |
