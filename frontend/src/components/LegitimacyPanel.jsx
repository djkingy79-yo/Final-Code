...
const LegitimacyPanel = ({ scores }) => {
  ...
  return (
    <div data-testid="legitimacy-panel" className="mt-3 rounded-lg border border-slate-200 p-2 sm:p-3 text-xs sm:text-sm bg-slate-50">
      <div className="font-bold mb-1.5 text-slate-800 text-xs sm:text-sm">Appellate Viability Assessment</div>
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-1.5 sm:gap-2 text-slate-700">
        <div className="p-1.5 sm:p-2 bg-white rounded border border-slate-100">
          <div className="text-[10px] sm:text-xs text-slate-500 mb-0.5">Outcome Impact</div>
          <div className="font-bold text-xs sm:text-sm">{scores.outcome_impact?.label || `${scores.viability_score || 0}/3`}</div>
        </div>
        <div className="p-1.5 sm:p-2 bg-white rounded border border-slate-100">
          <div className="text-[10px] sm:text-xs text-slate-500 mb-0.5">Legal Alignment</div>
          <div className="font-bold text-xs sm:text-sm">{scores.legal_alignment?.label || `${scores.legal_score || 0}/3`}</div>
        </div>
        <div className="p-1.5 sm:p-2 bg-white rounded border border-slate-100">
          <div className="text-[10px] sm:text-xs text-slate-500 mb-0.5">Evidence Support</div>
          <div className="font-bold text-xs sm:text-sm">{scores.evidence_support?.label || `${scores.evidence_score || 0}/3`}</div>
        </div>
      </div>
      <div className={`mt-1.5 sm:mt-2 p-1.5 sm:p-2 rounded border font-bold text-xs sm:text-sm ${viabilityColor}`}>
        ...
        <span className="text-[10px] sm:text-xs font-normal ml-2 opacity-70">({scores.total_score || 0}/9)</span>
      </div>
